...
class TestMeetingSynchronization(PmPortalDemoFunctionalTestCase):
    def test_sync_meeting_data(self):
        meeting = sync_meeting_data(self.amityville, self.json_meeting.get("items")[0])
        self.assertTrue(
    ...
    def test_sync_meeting_items(self):
        ...
        json_items = {
            "items": [
                item
                for item in self.json_meeting_items.get("items")
        ...
        self.assertListEqual(
            item_external_uids, [item.plonemeeting_uid for item in meeting.values()]
        )
        api.content.delete(objects=meeting.values())
        # all items
        results = sync_items_data(meeting, self.json_meeting_items, self.amityville)
        ...
        self.amityville.delib_category_field = "classifier"
        results = sync_items_data(
            meeting, self.json_meeting_items, self.amityville, True
        )
    ...
    def test_force_sync_item(self):
        meeting = sync_meeting_data(self.amityville, self.json_meeting.get("items")[0])
        results = sync_items_data(
            meeting, self.json_meeting_items, self.amityville, force=True
        )
        decision = {"content-type": "text/html", "data": u"<p>Nouvelle décision</p>"}
        self.json_meeting_items.get("items")[0].get("decision").update(decision)
        results = sync_items_data(
    ...
    def test_sync_annexes_publishable_enabled(self):
        self.amityville.info_annex_formatting_tal = "python: json['category_title']"
        ...
        sync_annexes_data(self.item, self.amityville, annexes_json)
        ...
        self.assertEqual(len(self.item.listFolderContents()), 1)
        sync_annexes_data(
            self.item, self.amityville, self.json_annexes_publishable_updated_mock
        )
        self.assertEqual(len(self.item.listFolderContents()), 1)
        file = self.item.listFolderContents()[-1]
        self.assertEqual(file.portal_type, "File")
    ...
    def test_sync_annex_title_tal_expr(self):
        ...
        sync_annexes_data(self.item, self.amityville, annexes_json)
        self.assertEqual(annex.title, "0s57")
        sync_annexes_data(self.item, self.amityville, annexes_json, force=True)
        self.assertEqual(annex.title, "Annexe")

    def test_sync_not_mapped_groups_in_charge_are_ignored(self):
        self.amityville.representatives_mappings = [
            {
        ...
        meeting = sync_meeting_data(self.amityville, self.json_meeting.get("items")[0])
        sync_items_data(meeting, self.json_meeting_items, self.amityville)

        self.assertEqual(
            ["dummy_mapped_uid_1", "dummy_mapped_uid_2"],
            meeting.values()[0].representatives_in_charge,
        )
        self.assertEqual(
            ["dummy_mapped_uid_1", "dummy_mapped_uid_2"],
            meeting.values()[0].long_representatives_in_charge,
        )
        self.assertEqual(
            ["dummy_mapped_uid_1"], meeting.values()[1].representatives_in_charge
        )
        self.assertEqual(
            ["dummy_mapped_uid_1"], meeting.values()[1].long_representatives_in_charge
        )
        ...
        self.assertEqual(
            ["dummy_mapped_uid_2", "dummy_mapped_uid_1"],
            meeting.values()[3].representatives_in_charge,
        )
        self.assertEqual(
            ["dummy_mapped_uid_2", "dummy_mapped_uid_1"],
    ...
    def test_item_title_formatting_tal(self):
        self.amityville.item_title_formatting_tal = (
            "python: '<h2>' + json['title'] + '</h2>'"
        )
        meeting = sync_meeting_data(self.amityville, self.json_meeting.get("items")[0])
        sync_items_data(meeting, self.json_meeting_items, self.amityville)
        self.assertEqual(
    ...
    def test_sync_items_number(self):
        ...
        items_brains = api.content.find(
            ...
            portal_type="Item",
            linkedMeetingUID=self.meeting.UID(),
        )
    ...
    def test_response_is_not_200(self):
        when(requests).get(
            "fake_url",
            auth=(self.amityville.username, self.amityville.password),
            headers=API_HEADERS,
        ).thenReturn(mock({"status_code": 500}))

        with self.assertRaises(ValueError) as ws_err:
            _call_delib_rest_api("fake_url", self.amityville)
