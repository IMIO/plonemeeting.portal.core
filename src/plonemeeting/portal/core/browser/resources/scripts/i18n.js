            ...
            if (packageJson.addons) {
                registry.addonNames.forEach((addon) => {
                    ...
                    if (fs.existsSync(addonlocale)) {
                        const addonItems = Pofile.parse(fs.readFileSync(addonlocale, "utf8")).items;
 ...
 */
function syncPoByPot() {
    const pot = Pofile.parse(fs.readFileSync("locales/plonemeeting.portal.core.pot", "utf8"));
...
function main() {
    ...
    const oldPot = fs
        .readFileSync("locales/plonemeeting.portal.core.pot", "utf8")
        .replace(/"POT-Creation-Date:(.*)\\n"/, "");

    if (newPot !== oldPot) {
        fs.writeFileSync(
            "locales/plonemeeting.portal.core.pot",
